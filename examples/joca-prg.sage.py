...
# This file was *autogenerated* from the file examples/joca-prg.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_10 = Integer(10)# -*- mode: python -*-
#
...
import sys, os, time
sys.path.insert(_sage_const_0 , os.path.join(os.path.dirname(os.path.abspath(__file__ if '__file__' in dir() else '.')), '..'))
sys.path.insert(_sage_const_0 , os.path.expanduser('~/parametric-rg'))
...
PDE = -int(_sage_const_1 )/int(_sage_const_2 )*(Psi[x, x] + Psi[y, y] + Psi[z, z])*r - Psi - E*r*Psi
...
ansatz = [Psi[x] - DPsi * v[x],
          ...
          r**_sage_const_2  - x**_sage_const_2  - y**_sage_const_2  - z**_sage_const_2 ]
...
def build_system_of_equations(eqn, consts):
    ring = eqn.parent()
    system = dict()
    nonconst_sub = tuple(_sage_const_1  if ring.gen(n) in consts else ring.gen(n)
                         for n in range(ring.ngens()))
    for coeff, monomial in eqn:
        ncp = monomial(nonconst_sub)
        cp = coeff * monomial // ncp
        system[ncp] = system.get(ncp, _sage_const_0 ) + cp
...
for p in joca_primes:
    ...
    inside = all(Ip.reduce(e) == ConstRing(_sage_const_0 ) for e in eqns_cr)
...
if os.environ.get('PRG_RUN', '1') == '1':
    ...
    try:
        systems = solver.MainProc(ansatz, [], max_vertices=BUDGET,
                                  wall_timeout=WALL, progress_every=_sage_const_10 )
        dt = time.time() - t0
        print("parametric RG: %d regular systems (%.1fs)" % (len(systems), dt))
        for idx, (A, S, N, W) in enumerate(systems):
            chain = [p for p in A if sympy.sympify(p) != _sage_const_0 ]
            try:
                _, prem = DiffRing.differential_prem(PDE, chain) if chain else (_sage_const_1 , PDE)
            except Exception:
                prem = None
            ...
            redundant = (prem_nf is not None and sympy.expand(prem_nf) == _sage_const_0 )
